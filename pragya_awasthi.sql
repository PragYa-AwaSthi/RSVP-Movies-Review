...
-- comment: out of 8 columns, 4 are null (country, world_gross_income, language, production_company) in movie table.
-- ----------------------------------------------------------------------------------------------------------------------------------
...
-- comment: no. of movies published are decresed with increase in year
-- ---------------------------------------------------------------------------------------------------------------------------------
...
-- comment: least no. of movies are published in month of december and highest in month of march.
-- ----------------------------------------------------------------------------------------------------------------------------------
...
select count(distinct id) as movies_produced from movie where (country like '%USA%' or country like '%india%') and  year = '2019';

-- comment: more than 1000 movies are published
-- -----------------------------------------------------------------------------------------------------------------------------------------
...
select count(*) as number_of_movies, genre from genre group by genre order by number_of_movies desc limit 1;

-- comment: highest movie production is of 'Drama' genre.
-- ---------------------------------------------------------------------------------------------------------------------------------
...
WITH genre_count AS(SELECT movie_id, count(genre) as count_of_genre FROM genre group by movie_id)
...
SELECT genre, avg(duration) AS avg_duration FROM genre AS f
	INNER JOIN movie AS c ON f.movie_id = c.id GROUP BY genre order by avg_duration;
    
-- comment: 'action' genre is with highest average duration whereas 'horror' is of least duration among all.
-- -------------------------------------------------------------------------------------------------------------------------------
...
SELECT genre, count(movie_id) as movie_count, dense_rank() 
	...
	AS 'genre_rank' FROM genre where genre = 'thriller' group by genre;  

-- comment: 'thriller' got 1st rank by using both dense_rank() and rank() function.
-- --------------------------------------------------------------------------------------------------------------------------------
...
SELECT m.title, r.avg_rating , rank() over (order by r.avg_rating desc) as 'movie_rank'
	from movie m inner join ratings r on m.id = r.movie_id limit 10;

-- comment: movies named 'kirkit' and 'love in kilnerry' got first rank with hightes rating of 10 each.
-- ---------------------------------------------------------------------------------------------------------------------------------
...
-- comment: highest median rating is 7.
-- --------------------------------------------------------------------------------------------------------------------------------
...
SELECT m.production_company, count(r.avg_rating) as movie_count , dense_rank() over (order by count(r.avg_rating) desc) as 'prod_company_rank'
	from movie m inner join ratings r on m.id = r.movie_id
    where avg_rating > 8  and production_company is not null 
    group by production_company ; 
    
-- comment: production companies named 'Dream Warrior Pictures' and 'National Theatre Live' got first rank.
-- ------------------------------------------------------------------------------------------------------------------------------------
...
SELECT genre, count(id) as movie_count FROM ratings AS t
	inner join movie AS c ON t.movie_id = c.id 
    inner join genre as g on g.movie_id = c.id
...
-- comment: genre 'drama' are the highest among all kind.
-- -----------------------------------------------------------------------------------------------------------------------------------
...
SELECT title, avg_rating, genre FROM ratings AS t
	inner join movie AS c ON t.movie_id = c.id 
    inner join genre as g on g.movie_id = c.id
...
SELECT count(id) as movie_num, median_rating FROM ratings AS t
	inner join movie AS c ON t.movie_id = c.id 
...
SELECT country, sum(total_votes) FROM ratings AS t
	inner join movie AS c ON t.movie_id = c.id 
...
-- comment: only name column is with 0 null values.
-- ----------------------------------------------------------------------------------------------------------------------------------
...
INNER JOIN _3_top_genre using (genre)
INNER JOIN ratings using (movie_id)
WHERE avg_rating > 8 GROUP BY name ORDER BY movie_count DESC limit 3 ;
 
 -- comment: 'James Mangold' is the top most director among 'James Mangold', 'Anthony Russo and 'Soubin Shahir'.
 -- ----------------------------------------------------------------------------------------------------------------------------------
...
-- comment: actors named 'Mammootty' and 'Mohanlal' got the highest median rating, are most liked.
-- ------------------------------------------------------------------------------------------------------------------------------------
...
SELECT m.production_company, r.total_votes as vote_count , rank() over (order by r.total_votes desc) as 'prod_comp_rank'
	from movie m inner join ratings r on m.id = r.movie_id
    where production_company is not null 
    group by production_company limit 3; 

-- comment: 'Marvel Studios' is the most liked/ popular production company.
-- --------------------------------------------------------------------------------------------------------------------------------
...
WITH actor_summary AS (SELECT n.NAME AS actor_name, total_votes, Count(r.movie_id) AS movie_count,
	...
	FROM movie AS m
	INNER JOIN ratings AS r ON m.id = r.movie_id
	INNER JOIN role_mapping AS s ON m.id = s.movie_id
	INNER JOIN names AS n ON s.name_id = n.id
...
WITH actress_summary AS (SELECT n.NAME AS actress_name, total_votes, Count(r.movie_id) AS movie_count,
...
WITH thriller_movies AS (SELECT DISTINCT title, avg_rating
	...
	INNER JOIN genre AS g using(movie_id) WHERE  genre LIKE 'THRILLER')
SELECT *,
       CASE
         ...
         ELSE 'Flop movies' 
         END 
	AS avg_rating_category FROM  thriller_movies order by avg_rating; 

-- comment: there are huge number of 'one-time-watched movies' followed by 'flop movies'.
-- -----------------------------------------------------------------------------------------------------------------------------------
...
WITH top_genres AS (
	...
	FROM movie AS m INNER JOIN genre AS g ON g.movie_id = m.id
	INNER JOIN ratings AS r ON r.movie_id = m.id
	WHERE avg_rating > 8 GROUP BY genre limit 3 ), movie_summary AS
(
...
WHERE  movie_rank<=5 ORDER BY YEAR;
...
SELECT m.production_company, count(r.movie_id) as movie_count , dense_rank() over (order by count(r.movie_id) desc) as 'prod_comp_rank'
	from movie m inner join ratings r on m.id = r.movie_id
    where median_rating >= 8 and production_company is not null 
    group by production_company limit 2; 
    
-- comment: 'star cinema' and 'A24' are the multilingual production companies with highest hits.
-- -----------------------------------------------------------------------------------------------------------------------------------
...
(
	...
	WHERE category = 'ACTRESS' AND avg_rating > 8 AND genre = "Drama" GROUP BY name )
SELECT *, Rank() OVER(ORDER BY movie_count DESC) AS actress_rank FROM actress_summary LIMIT 3;

-- comment: 'Parvathy Thiruvothu', 'Susan Brown' and 'Amanda Lawrence' are the three.
-- ----------------------------------------------------------------------------------------------------------------------------------
